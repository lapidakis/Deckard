...
6. **Per-token SessionHolder** owns its own `MCP.Server` instance and `StatelessHTTPServerTransport`. The boot-time `AuthContext` baked into the Server is just a fallback — the TaskLocal override is what lands in the audit row in HTTP-served calls.
7. **No session state.** The stateless transport issues no `Mcp-Session-Id`, and `initialize` is idempotent (`MCPHostBuilder.registerIdempotentInitialize`, installed after `server.start()` because start re-registers the SDK defaults). Any client may (re-)initialize at any time; every request stands alone. GET/DELETE return 405 — Deckard's tools are strict request/response, so there is no SSE stream to lose.
